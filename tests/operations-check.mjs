...
const dedupMigration = read("migrations/0002_lead_dedup_and_trash.sql");
...
const trashApi = read("functions/api/ops/trash.js");
const trashDetailApi = read("functions/api/ops/trash/[claimCode].js");
const softDeleteApi = read("functions/api/ops/leads/[claimCode]/trash.js");
const duplicateApi = read("functions/api/ops/leads/[claimCode]/duplicates.js");
...
check("2. All required services and statuses are whitelisted", ["open_gym","reformer_pilates","boxing","group_classes"].every((item) => SERVICES.includes(item)) && ["new","wechat_added","booked","visited","converted","duplicate","invalid","closed"].every((item) => STATUSES.includes(item)));
check("3. Valid anonymous lead payload passes", validateCreatePayload({ service:"reformer_pilates", language:"zh-CN", source:"flyer", store:"taizhou", campaign:"taizhou-opening-2026", pagePath:"/", requestId:"meg_test_ABC234", clientId:"123e4567-e89b-42d3-a456-426614174000" }).ok);
check("3b. Client ID is optional but strictly validated when present", validateCreatePayload({ service:"boxing", language:"en" }).ok && !validateCreatePayload({ service:"boxing", language:"en", clientId:"not-a-uuid" }).ok);
...
check("12b. Both languages share one local anonymous client ID with a safe fallback", app.includes('var key = "megAnonymousClientId"') && app.includes("randomUUID") && app.includes("getRandomValues") && app.includes("clientId:getAnonymousClientId()") && ![zh,en].some((html) => html.includes("megAnonymousClientId")));
...
check("22. New migration adds dedup, grouping, trash and operation audit fields", ["client_id","suspicious_group_id","status_before_delete","deleted_at","deleted_by","delete_reason","lead_operation_history"].every((value) => dedupMigration.includes(value)) && !/phone|mobile|wechat|ip_address|gender|health/i.test(dedupMigration));
check("23. Dashboard exposes duplicate labels, trash controls and permanent confirmation", opsHtml.includes('id="trashSection"') && opsHtml.includes('value="duplicate"') && opsJs.includes("疑似重复") && opsJs.includes('data-quick-status="duplicate"') && opsJs.includes('data-trash-action="purge"') && opsJs.includes("confirmation:entered"));
check("24. Every new Operations route keeps Access and origin authorization", [trashApi,trashDetailApi,softDeleteApi,duplicateApi].every((source) => source.includes("authorizeOps") && source.includes("validateOrigin")));
