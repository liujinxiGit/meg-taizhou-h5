...
const checks = [
  ["1. Mobile goal selector uses two columns", mobileCss.includes(".program-tabs,.goal-buttons,.why-grid{grid-template-columns:repeat(2,minmax(0,1fr))")],
  ["2. Mobile specialist program entry uses two columns", mobileCss.includes(".program-tabs,.goal-buttons,.why-grid")],
  ["3. WHY MEG uses two columns on mobile", mobileCss.includes(".why-grid{grid-template-columns:repeat(2") || mobileCss.includes(".program-tabs,.goal-buttons,.why-grid{grid-template-columns:repeat(2")],
  ...
  ["9. Bilingual toast messages exist", ["话术已复制", "Message Copied", "地址已复制", "Address Copied", "微信号已复制", "WeChat ID Copied"].every(text => app.includes(text))],
  ...
  ["19. Existing claim buttons remain", (zh.match(/js-claim-experience/g) || []).length === 3 && (en.match(/js-claim-experience/g) || []).length === 3],
  ...
  ["Hero image is preloaded", [zh, en].every(html => html.includes('rel="preload" as="image"') && html.includes("hero.webp?v=20260804-4"))],
  ["Language switch preserves source", app.includes('languageUrl.searchParams.set("source", source)')],
  ["Static resources share cache version 20260804-4", [zh, en].every(html => html.includes("/styles.css?v=20260804-4") && html.includes("/app.js?v=20260804-4"))]
];
